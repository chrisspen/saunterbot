...
void loop() {
    ...
    if(digitalRead(START_PIN)){
        for(pos = MIN_POSITION+SUB_POSITION; pos < MAX_POSITION-SUB_POSITION; pos += SERVO_STEP)  // goes from 0 degrees to 180 degrees
        {                                  // in steps of 1 degree
            myservo.SERVO_WRITE(pos);              // tell servo to go to position in variable 'pos'
            delay(SERVO_DELAY);                       // waits 15ms for the servo to reach the position
        }
        for(pos = MAX_POSITION-SUB_POSITION; pos >= MIN_POSITION+SUB_POSITION; pos -= SERVO_STEP)     // goes from 180 degrees to 0 degrees
        {
            myservo.SERVO_WRITE(pos);              // tell servo to go to position in variable 'pos'
            delay(SERVO_DELAY);                       // waits 15ms for the servo to reach the position
        }        
    }else{
        myservo.SERVO_WRITE(CENTER_POSITION);
    }
