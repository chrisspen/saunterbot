...
class Simulator(object):
    ...
    # def drawcircle(self, image, colour, origin, radius, width=0):
        # if width == 0:
            # pygame.draw.circle(image, colour, origin, int(radius))
        # else:
            # if radius > 65534/5:
                # radius = 65534/5
            # circle = pygame.Surface([radius*2+width, radius*2+width]).convert_alpha() # pylint: disable=too-many-function-args
            # circle.fill([0, 0, 0, 0])
    ...
    def reset_bodies(self):
        ...
        # _color = random.choice(list(THECOLORS.values()))
    ...
    def main(self):
        ...
        # body.position = (self.display_size[0]/2, self.ground_y+box_height/2+leg_length)
        # body.start_position = Vec2d(body.position)
        # body.start_angle = body.angle
        ...
        # body = pymunk.Body(mass, moment)
        # body.position = (self.display_size[0]/2-box_width/2+radius, self.ground_y+radius)
        # body.start_position = Vec2d(body.position)
        # body.start_angle = body.angle
        ...
        # self.space.add(body, shape)
        # # Fix foot to ground.
        # pj = pymunk.PinJoint(self.space.static_body, body,
            # (self.display_size[0]/2-box_width/2+radius, self.ground_y),
            # (0, 0))
        # self.space.add(pj)
        ...
        while running:
            ...
            ### Flip screen
            self.screen.blit(font.render("fps: " + str(clock.get_fps()), 1, THECOLORS["white"]), (0, 0))
            self.screen.blit(font.render("Press left mouse button and drag to interact", 1, THECOLORS["darkgrey"]), (5, height - 35))
            self.screen.blit(font.render("Press R to reset, any other key to quit", 1, THECOLORS["darkgrey"]), (5, height - 20))
